...
The distribution of samples among experimental conditions is as follows:  \\
...
xtable(reshape.counts, digits=0)
@
...
<<my.conds, echo=FALSE, results='asis'>>=
item2print <- NULL
dd <- 0 # global counter for Delta
for (ii in 1:length(wCont)) {
  cat(paste0("\\item Effect of condition ", compGroupName[ii],":       "))
  cat("    \\begin{enumerate} ")
    for (jj in 1:length(wCont[[ii]])) {
      dd <- dd +1 # raise one value the global counter for delta
      cat(paste0("\\item ", my.conds[[ii]][jj], " vs ", my.conds[[ii]][jj+1], " = ", my.conds[[ii]][jj+1], " - ", my.conds[[ii]][jj], " = $\\Delta _", dd, "$ "))
    }
  cat("    \\end{enumerate} ")
}
#my.conds.units
